...
> ⚠️ **重要提示**：LinuxDO 功能**仅支持本地运行**，不支持 GitHub Actions！
> 
> 原因：LinuxDO 使用 Cloudflare 防护，GitHub Actions 的数据中心 IP 会被 Cloudflare 拦截，无法通过人机验证。
...
### GitHub Actions (NewAPI 签到)

GitHub Actions 默认每天运行 2 次（北京时间 8:00 和 20:00）。

可在 `.github/workflows/newapi-check-in.yml` 中修改 cron 表达式。
